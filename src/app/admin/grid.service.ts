...
export class GridService {
    ...
    keysName_details = [];
    colTitle_details = [];
    config ={ "export":false,"export_id":0, "details_activated": false, "group": false, removable:"false"}
    ...
    delete_record(user_id){
        ...
        console.log(user_id)
        let body = JSON.stringify({"_id": user_id, "token": localStorage.getItem("token") });
        console.log(body);
        const headers = new HttpHeaders({'Content-Type': 'application/json'});
        let completeUrl = GlobalVariable.BASE_URL + 'to_delete_record';
    ...
    updateCheckbox(value,_id,master,app_name, field_name){
        ...
        let completeUrl = GlobalVariable.BASE_URL + 'update_checkbox';
        return this._http.post(completeUrl, body, {headers: headers})
